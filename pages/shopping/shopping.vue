	...
	<view class="shopping">
		...
		<scroll-view scroll-y="true" class="shopping-scroll" @scroll='scrollNumbers' :scroll-top='scrollTop'>
			...
			<view class="shopping-swipe">
				<swiper class="screen-swiper shopping-swipe-box" :indicator-dots="true" :circular="true" :autoplay="true" interval="5000"
				 duration="500">
					<swiper-item v-for="(item,index) in goods.carousel_image" :key="index">
			...
			<view class="shopping-news">
				...
				<view class="shopping-news-prick">
					<view class="shopping-news-prick-left">
						...
						<text class="shopping-news-prick-left-newprick">{{goods.price}}</text>
						<text v-if="goods.price_unit" class="shopping-news-prick-left-company">{{goods.price_unit}}</text>
						<text v-if="parseFloat(goods.origin_price) > 0" class="shopping-news-prick-left-oldprick">¥{{goods.origin_price}}</text>
					</view>
					<view class="shopping-news-prick-right">
						月销量 {{goods.mouth_sale}}
					</view>
				</view>
			</view>
			<!-- 配送 -->
...
<script>
	export default {
		...
		onLoad(option) {
			this.goodsInfo(option.goodsId)
		},
		methods: {

			goodsInfo(goodsId) {
				const _this = this
				_this.get('/goods/info', {
					goods_id: goodsId
				}).then(res => {
